...
- React 只是来源外壳，demo / 截图 / JSX 中呈现出来的视觉效果才是还原目标
...
5. 把第三方 React 依赖改成项目已有方案，或用中性占位表达其视觉和交互角色；前提是视觉效果不能被随意改味。
...
- 任何会影响显隐、布局、视觉层次、选中态、禁用态、错误态的状态都属于“影响界面”，不能误删。
...
- 不要因为项目里已有“差不多”的弹窗、面板、表单、按钮组件，就直接替换来源实现；只有视觉等价时才允许复用。
