...
export const useAppStore = defineStore('app', () => {
  ...
  const messages = ref<{ role: 'user' | 'agent'; content: string }[]>([
    {
      role: 'agent',
      content: '你好！我是你的智能代理。请点击上方按钮配置我的工作流，或直接告诉我你的需求。',
    },
  ])
  ...
  const configNodes = ref<Node[]>([
    {
      ...
      type: 'custom',
      position: { x: 250, y: 50 },
      data: { label: 'Start', type: 'start', config: { description: 'Entry point' } },
    },
    {
      id: 'c2',
      type: 'custom',
    ...
    },
    {
      ...
      data: {
        label: 'RAG Search (PubMed)',
        type: 'tool',
        config: { source: 'pubmed', max_results: 10 },
      },
    },
    {
      id: 'c4',
      ...
      data: {
        ...
        type: 'tool',
        config: { server: 'filesystem-mcp', mode: 'read-only' },
      },
    },
    {
      id: 'c5',
      ...
      data: { label: 'Decision Node', type: 'decision', config: { threshold: 0.8 } },
    },
  ])
