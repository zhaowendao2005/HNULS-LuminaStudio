          ...
          <details class="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" open>
            ...
            <div class="mt-3 grid gap-4 lg:grid-cols-2">
              <div class="space-y-2">
                ...
                <pre
                  class="max-h-80 overflow-auto rounded-2xl bg-slate-950 p-4 text-xs leading-5 text-slate-100"
                  >{{ lastResolveRequestJson }}</pre
                >
              </div>
...
const lastResolveRequestJson = computed(() => {
  if (!knowledgeDebugStore.lastResolveRequest) {
    return '暂无请求'
  }

  return JSON.stringify(knowledgeDebugStore.lastResolveRequest, null, 2)
})

const lastSearchRequestJson = computed(() => {
  if (!knowledgeDebugStore.lastSearchRequest) {
    return '暂无请求'
  }

  return JSON.stringify(knowledgeDebugStore.lastSearchRequest, null, 2)
})
