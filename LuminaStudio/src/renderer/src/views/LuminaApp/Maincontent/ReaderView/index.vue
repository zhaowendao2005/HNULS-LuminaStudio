<template>
  <div class="ls-reader flex h-full gap-6 relative">
    ...
    <!-- PDF Area -->
    <div
      class="flex-1 bg-white/90 backdrop-blur-sm rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col relative z-10 transition-all duration-300 hover:shadow-md"
    >
      <div
        class="h-12 border-b border-slate-100 flex items-center px-4 justify-between bg-white/50"
      >
        ...
        <div class="flex gap-2 text-slate-400 flex-shrink-0">
          <svg
            class="w-4 h-4 flex-shrink-0 hover:text-emerald-600 cursor-pointer transition-colors"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
      ...
      <div
        class="flex-1 p-10 overflow-y-auto font-serif text-slate-800 leading-relaxed text-lg"
        @click="showAskTooltip = false"
      >
        <h1 class="text-3xl font-bold mb-4 font-sans text-slate-900">Abstract</h1>
        <p class="mb-6 text-slate-600">
          The CRISPR-Cas9 system has revolutionized genomic engineering. Originating from the
          adaptive immune system of Streptococcus pyogenes, this mechanism allows for precise
          cleavage of DNA sequences.
        </p>
        <p
        ...
        >
          Cas9 is a dual-RNA-guided DNA endonuclease that uses base pairing to recognize and cleave
          target DNAs.
          <span class="text-emerald-400 text-sm ml-2 font-sans opacity-60">[Select Text]</span>
        </p>
        <p class="mb-6 text-slate-600">
          Structural studies have revealed the distinct conformational states of Cas9 during the
          process of DNA interrogation and cleavage, highlighting the role of the PAM sequence in
          target recognition.
        </p>
