<template>
  ...
  >
    ...
    <button
      class="nc-model-pill-a9k2 flex min-w-0 items-center gap-1.5 justify-self-center rounded-full border border-gray-200 bg-white px-3 py-1.5 text-[14px] text-gray-700 shadow-sm transition-colors hover:border-gray-300 hover:bg-gray-50"
      type="button"
      :title="workspaceStore.currentTopicModelLabel"
      @click="workspaceStore.openModelSelector"
    >
...
import { ChevronDown, Search, Settings2, SplitSquareHorizontal } from 'lucide-vue-next'
...
const { currentAssistant, currentTopic } = storeToRefs(workspaceStore)
</script>
