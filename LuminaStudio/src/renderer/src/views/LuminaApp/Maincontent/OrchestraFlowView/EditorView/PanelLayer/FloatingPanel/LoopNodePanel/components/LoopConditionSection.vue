<template>
  <section class="of-doc-section">
    ...
    <div v-if="modelValue.length === 0" class="of-state-empty">暂无终止条件，循环会按最大轮次继续执行。</div>

    <div v-else class="of-condition-list">
      <div
        v-for="item in modelValue"
        :key="item.id"
        class="of-condition-entry"
      >
        ...
        <div class="of-doc-section">
            ...
            <Teleport to="body">
              ...
              >
                <CapsuleTooltip
                  ...
                  :text="getOperatorDescription(String(option.value) as OFIfElseConditionOperator, item.variable_type)"
                  placement="top"
                  max-width="260px"
                >
                  <button
            ...
            <div class="of-condition-line">
              ...
              <button
                ...
                @click="
                  emit('patch', item.id, {
                    compare_source_mode:
                      (item.compare_source_mode || 'constant') === 'variable' ? 'constant' : 'variable',
                    compare_selector:
          ...
          <div class="of-doc-line-soft">
            {{ logicalOperator === 'and' ? '需要同时满足其它条件后才终止。' : '满足任一条件即可终止。' }}
          </div>
        </div>
      </div>
    </div>
...
function getOperatorLabel(
  ...
  return operatorOptions({ operator, variable_type: type } as OFIfElseCondition).find(
...
function findOperatorTrigger(conditionId: string) {
  return operatorTriggerRefs.value.find((element) => element.dataset.conditionId === conditionId) || null
}
