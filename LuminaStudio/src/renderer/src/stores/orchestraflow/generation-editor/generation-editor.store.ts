...
export const useOrchestflowGenerationEditorStore = defineStore('orchestflow-generation-editor', () => {
  ...
  const analysisMessages = computed(() => {
    return currentSession.value?.messages.filter((item) => item.channelKey === 'analysis-planner') || []
  })
  ...
  const designMessages = computed(() => {
    return currentSession.value?.messages.filter((item) => item.channelKey === 'design-planner') || []
  })
  ...
  async function sendMessage(channelKey: GenerationChannelKey, text: string): Promise<void> {
    ...
    if (!trimmed) return
    const designDocumentId = channelKey === 'design-planner' ? activeDesignDocument.value?.id || null : null
    requireData(
  ...
  async function updateGlobalSettings(settings: Partial<GenerationGlobalSettings>): Promise<void> {
    globalSettings.value = requireData(
