...
import { VerifyCopilotDataSource } from './verify-copilot.datasource'

export const useGenerationVerifyCopilotStore = defineStore('of-generation-verify-copilot', () => {
  const input = ref('')
  ...
  function getMessages(detail: GenerateSessionDetailViewModel | null) {
    return detail?.messagesByChannel['verify-copilot'] ?? []
  }
  ...
  async function sendMessage(
    ...
    try {
      const result = await VerifyCopilotDataSource.sendMessage({
        sessionId: detail.id,
        channelKey: 'verify-copilot',
      ...
      const target = detail.messagesByChannel['verify-copilot'].find(
