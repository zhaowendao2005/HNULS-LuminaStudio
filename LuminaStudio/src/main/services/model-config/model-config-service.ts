...
export class ModelConfigService {
  ...
  constructor(databaseManager: DatabaseManager) {
    ...
    this.ensureApiModeColumn()
  }

  private ensureApiModeColumn(): void {
    const columns = this.db.prepare("PRAGMA table_info('model_providers')").all() as Array<{ name: string }>
    if (columns.some((column) => column.name === 'api_mode')) return
    this.db.prepare("ALTER TABLE model_providers ADD COLUMN api_mode TEXT NOT NULL DEFAULT 'auto'").run()
  }
    ...
    try {
      ...
      for (const row of providerRows) {
        ...
        providers.push({
          ...
          apiMode: (row.api_mode || 'auto') as ModelProviderApiMode,
    ...
    try {
      const transaction = this.db.transaction(() => {
        ...
        if (patch.activeProviderId !== undefined) {
          this.db
            .prepare(`INSERT OR REPLACE INTO app_settings (key, value) VALUES ('activeProviderId', ?)`)
  ...
  async syncModels(providerId: string): Promise<RemoteModelGroups> {
    try {
      ...
      if (!providerRow) throw new Error(`Provider not found: ${providerId}`)
      ...
      return this.groupModels(data.data || [])
    } catch (error) {
  ...
  private inferGroupFromModelId(modelId: string): string {
    ...
    if (parts.length >= 3 && parts[0] && parts[1]) return `${parts[0]}-${parts[1]}`
    return DEFAULT
  }
