...
function start() {
  ...
  if (!Array.isArray(commandAndArgs) || commandAndArgs.length === 0) {
    console.error(
      '[logger] No command specified. Usage: node scripts/run-with-logger.mjs --name <name> -- <command> [args...]'
    )
    process.exit(1)
  }
  ...
  child.on('error', (error) => {
    ...
    try {
      fileStream.write(cleanForFile(`${message}\n`))
    } catch {}
  })
  ...
  child.on('close', (code, signal) => {
    ...
    try {
      fileStream.write(cleanForFile(endLine))
    } catch {}
    fileStream.end(() => {
